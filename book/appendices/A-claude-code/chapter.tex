...
A minimal Workflow script that scores three chapters concurrently illustrates
the core pattern:
...
export const meta = {
  ...
  phases: [{ title: 'Score' }],  // declares progress group shown in /workflows
}
...
// parallel() fans out: all three agents start immediately and run concurrently.
...
// results is an array: [{ clarity: 8, rigor: 7, ... }, { ... }, { ... }]
return results
\end{minted}

Running this script with \texttt{/workflows} open shows three concurrent
``score:ch-name'' agents in the \emph{Score} group.  Each returns a validated
JSON object matching \texttt{SCORE\_SCHEMA}; if a model response does not
conform to the schema, the runtime retries automatically before failing.
...
\begin{example}[End-to-end: drafting a literature survey with Claude Code]
\label{ex:claude-code-survey}
Suppose a researcher wishes to survey ten recent SSRN working papers on LLM
applications in credit risk.  The following session, executed inside the
project repo, illustrates a complete workflow.

\begin{minted}{text}
# Step 1: Create the scaffold
/new-topic  appendix B on credit-risk LLM survey

# Step 2: Invoke the SSRN researcher agent on the new appendix
/ssrn-enrich appB

# Step 3: Draft with the book-writer and literature-reviewer agents
/draft-chapter A-credit-risk-survey

# Step 4: Score and refine until the 8/10 threshold is met
/score-content book/appendices/B-credit-risk-survey/chapter.tex
/refine-until-threshold book/appendices/B-credit-risk-survey/chapter.tex

# Step 5: Run quality gates and release
/release-chapter B-credit-risk-survey
\end{minted}

Each slash command is a skill that chains a sequence of specialised agents.
The researcher reviews the plan at each stage (Plan mode) before approving
execution; the full pipeline from scaffold to release typically runs in a
