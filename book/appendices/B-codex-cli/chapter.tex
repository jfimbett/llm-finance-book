...
The release was deliberate counterpart positioning: within days of Anthropic's
...
Codex CLI enforces safety at the operating-system kernel layer, which means
that even if the model requests a forbidden operation, the kernel will refuse
it before any harm occurs.  For financial institutions with strict data
governance requirements, this distinction matters.
\end{context}
...
\begin{description}
  ...
  \item[Linux --- Landlock] A Linux Security Module (available since kernel
        ...
        mechanism.  Codex uses Landlock to restrict file-system access to the
...
ephemeral virtual machine on OpenAI's infrastructure.  The VM is pre-loaded
with the repository (fetched from GitHub or supplied as a tarball) and
destroyed after the task completes.  Benefits for financial workflows:
