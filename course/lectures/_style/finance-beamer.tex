...
% --- Fonts: match the book (Palatino / Pazo Math) ---------------------
% The book uses mathpazo (Palatino text + Pazo maths). We mirror that here
% so the slides read like the book rather than Beamer's default sans.
\usepackage[T1]{fontenc}
\usepackage{mathpazo}
\usefonttheme{professionalfonts}   % keep our maths font; no Beamer substitution
\usefonttheme{serif}               % use Palatino (roman) for all slide text
\linespread{1.04}
\usepackage{microtype}
% Frame/section titles: bold Palatino (book-like headings).
...
% The base size is set to 9pt per deck (\documentclass[9pt]{beamer}); here we
% only tighten block spacing. Smaller, book-like text leaves room for content.
