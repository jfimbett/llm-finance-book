...
14\,GB of GPU memory for the weights alone---before accounting for optimizer
states, which add another 56\,GB for AdamW.  For most practitioners, this is
infeasible.  Parameter-efficient fine-tuning (PEFT) methods solve this by
...
It is the reference implementation for local LLM inference and is embedded
in most higher-level tools including Ollama.
...
The OpenAI-compatible interface means existing Python code that calls the
OpenAI API can be redirected to a local Ollama instance by changing two
lines---\texttt{base\_url} and \texttt{api\_key}---with no other
modifications.
...
TGI is Hugging Face's production-grade inference server.  It is the backend
for Hugging Face Inference Endpoints and is optimised for low tail latency in
