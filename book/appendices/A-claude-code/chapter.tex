...
\begin{enumerate}
  \item Install Claude Code, authenticate it, and configure a project-level
        \texttt{CLAUDE.md} and \texttt{settings.json} with appropriate
        permission and deny-list entries.
  \item Describe the built-in tool set (Read, Write, Edit, Bash, Glob, Grep,
        Agent, Workflow, Skill) and explain which tool to use for a given task.
  \item Write a \texttt{SKILL.md} that orchestrates two or more agents in
        sequence, and register it as a slash command.
  \item Configure a \texttt{PostToolUse} hook that runs a linter after any
        file edit.
  \item Design a Workflow script that fans work out across multiple agents in
        parallel and returns structured JSON results.
  \item Identify the security risks relevant to deploying Claude Code on
        financial data and apply the deny-list and secrets-management mitigations.
  \item Map at least three quantitative-finance research tasks to concrete
        Claude Code workflows involving skills, hooks, and scheduled routines.
\end{enumerate}
\end{remark}
...
42{,}896$\times$ growth rate \citep{anthropic2025claudecode}.  By early 2026,
Anthropic reported that 90\% of its own production code was AI-written,
predominantly via Claude Code.  The tool had graduated from curiosity to
infrastructure.
\end{context}
...
AI coding assistants that generate code from natural language prompts have been
studied since at least \citet{chen2021evaluating}, whose Codex model underpins
GitHub Copilot.  Controlled experiments subsequently showed productivity gains
of roughly 55\% for tasks assisted by such tools \citep{peng2023impact}.  What
distinguishes Claude Code from those earlier systems is architectural: the model
is placed \emph{in the loop of the build process} rather than acting as a
side-channel advisor.  When the model writes a function,
...
command (e.g., \texttt{/fetch-filings}, \texttt{/run-backtest}).

\subsection{Plan Mode}
\label{app:claude-code:plan-mode}

Plan mode is a read-only exploration mode: Claude Code can analyse the
codebase, reason about architecture, and produce a structured plan, but it
cannot execute any writes or shell commands.  Use Plan mode to understand what a
large, ambiguous task will require before granting execution permissions.
...
Claude Code loads and merges all three levels, giving the more specific file
...
YAML front matter (useful for storing project-level configuration such as data
paths, model parameters, or quality thresholds), conditional blocks, and even
embedded skill references.  Claude Code does not execute front matter
...
The \texttt{permissions.allow} list uses \texttt{Tool(pattern:*)} syntax;
the pattern matches against the tool's first argument.  The
\texttt{permissions.deny} list takes precedence over \texttt{allow}: if a
command matches both, Claude Code denies it.  This asymmetric design ensures that safety
...
---
name: project-data-conventions
description: Return convention for this project is log returns, not simple
metadata:
  type: project
---

Always compute log returns: `np.log(price / price.shift(1))`.
**Why:** Risk models and factor regressions assume additive returns; simple
returns compound multiplicatively and bias long-horizon estimates.
**How to apply:** Flag any code that uses `price.pct_change()` without a
subsequent log transformation.
\end{minted}
...
\begin{minted}{bash}
# Example: run a backtest in the background
python scripts/run_backtest.py --strategy momentum --start 2015-01-01
\end{minted}
...
\begin{itemize}
  ...
  \item \textbf{Composable} --- a skill can invoke other skills via the
        ...
        \texttt{/fetch-filings} → \texttt{/extract-sentiment} →
        \texttt{/build-factors}.
\end{itemize}

Skills live in \texttt{.claude/skills/}; run \texttt{/help} from any session
to list those available in the current project.
...
pattern for quantitative projects is a \texttt{PostToolUse} hook on \texttt{Edit}
that re-runs a data-validation script whenever a model configuration file
changes, surfacing schema errors before the next agent step.
...
\citep{anthropic2024mcp} that allows Claude Code to connect to external
processes that expose tools and data sources.  An MCP server is a long-running process that Claude Code discovers
...
Transformer itself.  Early demonstrations used independent agents with shared
memory to simulate complex social dynamics \citep{park2023generative}; more
recent work has evaluated LLM systems against real-world software engineering
tasks at scale \citep{jimenez2024swebench}.  A single model, no matter how
capable, operates within a fixed context window; a system of models can divide
a large task into pieces, process them in parallel, check each other's work, and
synthesise results that no single context window could hold.  Claude Code's
multi-agent architecture is designed around exactly this observation.
\end{context}
...
{
  "description": "Classify sentiment of an earnings call transcript",
  "prompt": "Read data/transcripts/AAPL_2024Q4.txt and classify overall ...",
  "schema": {
    "type": "object",
    "properties": {
...
\texttt{agentType: "risk-checker"}, Claude Code reads
\texttt{.claude/agents/risk-checker.md} and prepends it to the child's system
prompt.

A finance research project might define agent types such as:

\begin{itemize}
  \item \texttt{data-validator} --- checks that a dataset conforms to expected
        schema, return conventions, and date ranges; returns \texttt{PASS} or
        \texttt{FAIL} with a list of issues.
  \item \texttt{factor-builder} --- constructs Fama--French-style factor
        portfolios from cleaned price and fundamentals data.
  \item \texttt{risk-checker} --- verifies that a proposed portfolio does not
        breach position limits, sector concentration caps, or drawdown thresholds.
  \item \texttt{report-writer} --- drafts regulatory or client-facing reports
        from structured backtest output, following a house style guide.
  \item \texttt{literature-reviewer} --- scans SSRN and arXiv for recent papers
        on a given topic and returns structured BibTeX entries.
\end{itemize}
...
A minimal Workflow script that classifies the sentiment of multiple earnings
call transcripts concurrently illustrates the core pattern:

\begin{minted}{javascript}
export const meta = {
  name: 'classify-earnings-calls',
  description: 'Sentiment-classify earnings transcripts in parallel',
  phases: [{ title: 'Classify' }],  // progress group shown in /workflows
}

const tickers = ['AAPL', 'MSFT', 'JPM', 'GS', 'BAC']

// JSON Schema forces each agent to return structured data, not free text.
...
// parallel() fans out: all five agents start immediately and run concurrently.
// Wall-clock time = slowest single agent, not sum of all five.
const results = await parallel(tickers.map(t => () =>
  agent(
    `Read data/transcripts/${t}_2024Q4.txt. Classify sentiment and extract
     up to five key forward-looking phrases.`,
    { label: `classify:${t}`, phase: 'Classify', schema: SENTIMENT_SCHEMA }
  )
))

// results is an array of validated objects: [{ ticker: 'AAPL', sentiment: 'positive', ... }, ...]
return results
\end{minted}

Running this script with \texttt{/workflows} open shows five concurrent
``classify:TICKER'' agents in the \emph{Classify} group.  Each returns a
validated JSON object matching \texttt{SENTIMENT\_SCHEMA}; if a model response
does not conform to the schema, the runtime retries automatically before
failing.
...
\begin{minted}{text}
/schedule "every weekday 06:00 UTC: /fetch-factor-returns, validate schema, commit"
\end{minted}
...
\begin{itemize}
  \item Nightly factor return downloads from WRDS or Ken French's data library,
        with automatic schema validation before appending to the master dataset.
  \item Weekly model recalibration reports that re-estimate factor loadings,
        flag any Sharpe ratio regression, and post a summary to Slack.
  \item Post-merge checks that re-run all backtest notebooks and compare
        performance metrics against a stored baseline, alerting on regressions.
\end{itemize}
...
claude --non-interactive \
       --model claude-sonnet-4-6 \
       "Run the backtest suite on all modified strategy files in this PR. \
        Fail if any strategy's Sharpe ratio drops below 0.5."
\end{minted}
...
Table~\ref{tab:token-costs} gives approximate consumption for representative
finance research tasks.

\begin{table}[ht]
\centering
\caption{Approximate token consumption for representative Claude Code tasks (Sonnet 4.6).}
...
The following use cases have proven especially productive for quantitative
...
\begin{description}
  ...
  \item[Report drafting] Using a \texttt{report-writer} agent iteratively to
        produce investment memos or regulatory reports that meet a house style
        and a readability threshold, with a \texttt{PostToolUse} hook that
        checks compliance terminology on every edit.
...
\begin{example}[End-to-end: building an earnings-call sentiment factor]
\label{ex:claude-code-sentiment-factor}
Suppose a quant researcher wants to build a long/short equity factor based on
earnings-call sentiment for S\&P 500 firms.  The following session illustrates
a complete Claude Code workflow from raw transcripts to a factor return series.

\begin{minted}{bash}
# Step 1: Use Plan mode to review the task scope before touching any files
# (Press Shift+Tab to enter Plan mode, then submit the prompt)
"Outline a pipeline to classify Q4 2024 earnings calls for S&P 500 firms,
 construct a long-short sentiment factor, and backtest it 2015-2024."

# Step 2: Approve the plan, switch back to Edit mode, then run the fetch skill
/fetch-filings --source seekingalpha --quarter 2024Q4 --universe sp500

# Step 3: Fan out sentiment classification across all transcripts (Workflow)
/classify-sentiment --input data/transcripts/2024Q4/ --output data/sentiment/

# Step 4: Build the factor and run the backtest
claude "Using data/sentiment/2024Q4_scores.csv, construct a monthly
        long-short portfolio: long top-quintile sentiment, short bottom-quintile.
        Use equal weights within each leg. Compute net returns 2015-2024."

# Step 5: Validate and commit
/validate-backtest results/sentiment_factor_2015_2024.csv
git add data/sentiment/ results/ && git commit -m "feat: Q4-2024 sentiment factor"
\end{minted}

At Step 3, the \texttt{/classify-sentiment} skill launches a Workflow that
runs one sub-agent per transcript in parallel; wall-clock time for 500
transcripts is roughly equal to the slowest single classification rather than
the sum.  The researcher reviews the plan at each stage and uses the double-Esc
rewind to undo any step that produces unexpected output.
\end{example}
...
\begin{enumerate}
  \item \textbf{Plan before writing.}  Use Plan mode to review the proposed
        changes before committing to Auto-Accept.  For any new pipeline or
        feature, enter Plan mode first to inspect the intended file changes,
        then switch to Edit mode to execute.

  \item \textbf{Validate early, validate often.}  Run your validation skill
        after every significant change, not just at the end.  Structured JSON
        output from a validator agent provides a more reliable compass for the
        next iteration than subjective impression.
  ...
  \item \textbf{Commit checkpoints.}  Have skills commit after every major step.
        Frequent small commits make it easy to bisect a regression introduced
        by an agent and revert to a known-good state without losing work.
\end{enumerate}
