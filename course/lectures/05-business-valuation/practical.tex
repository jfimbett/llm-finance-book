...
\begin{enumerate}
  ...
  \item \textbf{Problem 1} (15 min) --- Audit an LLM's 10-K extraction; propagate the error to the DCF
...
\begin{block}{What you will practise}
  Auditing LLM-extracted financial data and propagating extraction error; structuring CoT
  forecasting prompts; parameterising and interpreting a Monte Carlo DCF; reasoning about
  how model error---not the formula---drives valuation uncertainty.
\end{block}
\end{frame}
...
% ============================================================
\begin{frame}
\frametitle{Problem 1: Audit an LLM's 10-K Extraction (15 min)}

You pointed an extraction LLM at a mid-cap industrial's FY2023 10-K. It returned the
following JSON (USD millions). \textbf{One field is mis-extracted} --- a classic LLM
failure: it copied CapEx from the cash-flow statement \emph{without} the parenthesis
that marks it as an outflow, and also flattened a footnote.
...
\textbf{The point is the extraction, not the arithmetic --- tasks on the next slide.}
\end{frame}

% ============================================================
% FRAME 5b: Problem 1 --- tasks
% ============================================================
\begin{frame}
\frametitle{Problem 1: Tasks (cont.)}

The extraction LLM's figures are on the previous slide.

\medskip
\textbf{Your tasks:}
\begin{enumerate}
  \item \textbf{Audit the extraction.} Which textual features of a 10-K make a line
        item like CapEx, parenthesised negatives, or footnoted figures error-prone for
        an LLM? Name the verification check (e.g.\ a sign/units cross-check against the
        cash-flow statement) you would add to catch it automatically.
  \item Using the (audited) figures, compute FCFF with the effective tax rate
        (infer from income tax / EBT); this $\text{FCFF}_0$ feeds the case study.
  \item \textbf{Propagate the error.} Recompute the DCF enterprise value
        (WACC $=10\%$, 5-yr growth $=6\%$, terminal growth $=2.5\%$) \emph{with} the
        mis-extracted CapEx vs.\ the corrected value. How large a valuation error does a
        single bad extraction cause?
  \item State a rule: at what extraction-confidence level would you block an automated
        valuation from reaching a portfolio manager without human review?
\end{enumerate}
\end{frame}
...
% ============================================================
\begin{frame}[fragile]
\frametitle{Case Study Step 1: Run the Simulation (1/2)}
...
Draw correlated parameter vectors, then clip terminal growth below WACC to keep
the Gordon terminal value finite. The valuation loop follows on the next slide.
\end{frame}

% ============================================================
% FRAME 8b: Case study --- step 1, code (continued)
% ============================================================
\begin{frame}[fragile]
\frametitle{Case Study Step 1: Run the Simulation (2/2)}
...
% ============================================================
\begin{frame}
\frametitle{Solutions: Problem 1 --- Extraction Audit and Error Propagation}

\textbf{1. The audit.} CapEx, parenthesised negatives, and footnoted figures are the
classic LLM extraction traps: the model drops the sign/parenthesis or flattens a note.
Fix: a verification sub-agent that cross-checks each extracted figure's \emph{sign and
units} against the cash-flow statement and flags disagreements.

\medskip
\textbf{2. FCFF (corrected, CapEx as outflow).} $\tau = 85/368 = 23.1\%$;
$\Delta\text{NWC}=65$:
\[ \text{FCFF} = 428(0.769) + 195 - 240 - 65 = \mathbf{219.3\text{ M}}\ (=\text{FCFF}_0). \]

\medskip
\textbf{3. Propagated error.} If the LLM treats CapEx as $+240$ (sign dropped),
$\text{FCFF}\to 699.3$ M --- a $\mathbf{3.2\times}$ overstatement that flows straight
into the DCF (corrected EV $\approx 3{,}672$ M vs.\ a grossly inflated figure). A single
mis-extracted field, not the discounting, breaks the valuation.

\medskip
\textbf{4. Rule.} Block any automated valuation whose extraction confidence is below your
threshold (or whose sign-check fails) from reaching a PM without human review.
\end{frame}
...
% ============================================================
\appendixstart{Stretch Problem: Sensitivity Decomposition}

\begin{frame}[noframenumbering]{Stretch Problem: Decompose the EV Variance}

For students who finish early, extend the Monte Carlo case study. Recall the LLM
forecasts the \emph{explicit growth path} (Problem 2's CoT step), so its forecast error
is the uncertainty that the model itself injects.

\medskip
\textbf{Goal:} quantify how much of the variance in enterprise value is driven by
each uncertain input (WACC, LLM-forecast explicit growth, terminal growth), and hence
how much the \emph{LLM's} forecast error contributes to valuation uncertainty.

\medskip
\textbf{Tasks:}
\begin{enumerate}
  \item Re-run the simulation three times, each time \emph{freezing} two of the
        three inputs at their mean and letting only one vary.
  \item Record $\mathrm{Var}(EV)$ for each single-input run; express each as a
        fraction of the all-inputs-varying variance.
  \item \textbf{Predict, then check:} which input dominates? Reconcile with the
        analytic result $\frac{\partial V}{\partial g} = \frac{\text{FCF}_n(1+\text{WACC})}{(\text{WACC}-g)^2}$.
\end{enumerate}

\medskip
\begin{block}{Expected finding}
Terminal-growth uncertainty should dominate, consistent with the 15--18\% EV error
per 100-bps $g$ shock from the recap. The decomposition makes the analytic
sensitivity tangible.
\end{block}
\end{frame}
